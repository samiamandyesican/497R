...
#=
I'm going to sweep across a range of displacements (translations) for the tailplane to examine tail volume ratios.
=#
...
min_tran = [chord[1]+1.0, 0.0, 0.0] # minimum transation of the tail
max_tran = [chord[1]+30.0, 0.0, 0.0] # maximum translation of the tail
iterations = 20 # number of iterations
step = step_it(min_tran, max_tran, iterations)
...
for i in 1:iterations
   ...
   # define translation of the tail 
   if i == 1
        global trans = min_tran
    else
        global trans += step
    end
    ...
    if i ==1
        write_vtk("wing-tail-short-h", surfaces, properties; symmetric)
    elseif i == iterations
        write_vtk("wing-tail-long-h", surfaces, properties; symmetric)
...
generate_plots(labels, data, "horizontal_movement_only")
